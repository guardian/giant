    ...
    renderRecipient(r, i) {
        ...
        return (
            <div className="email__recipient" key={r.email + i}>
                {
                    r.displayName
                    ? <HoverSearchLink q={r.displayName}/>
                    : false
                }
                {
                     r.email
                     ? <span className='email__recipient-address'>(<HoverSearchLink q={r.email}/>)</span>
                     : false
                }
            </div>
        );
        ...
        return (
            <React.Fragment>
                <div className='email__header-row'>
                    <h2>From:</h2>
                    <h3 className='email__header-item'>{this.renderRecipient(this.props.email.from)}</h3>
                </div>
                <div className='email__header-row'>
                    <h2>Sent:</h2>
                    <h3 className='email__header-item'>
                        {
                            ...
                            ?
                                hdate.prettyPrint(new Date(this.props.email.sentAt), {showTime: true})
                            :
                                '<Unknown>'
                        }
                    </h3>
                ...
                <div className='email__header-row'>
                    ...
                    <h3 className='email__header-item email__header-item--inline'>
                        {this.props.email.recipients.map((r, i)=> this.renderRecipient(r, i))}
                    </h3>
                </div>
                <div className='email__header-row'>
                    <h2>Subject:</h2>
        ...
        return (
            <div className='email__header'>
                <div className='btn-tab-group'>
                    <DetailsLink current={current} to='Summary' navigate={this.props.setDetailsView} />
                    <DetailsLink current={current} to='Thread' navigate={this.props.setDetailsView} />
                </div>
                {this.renderDetails()}
            </div>
        );
...
function DetailsLink({to, navigate }) {
    ...
    return <button className='btn btn--tab' onClick={onClick}>{to}</button>;
}
