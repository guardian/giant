...
export class DownloadModal extends React.Component<any, any> {
  static propTypes = {
    ...
    if (this.props.isOpen && !prevProps.isOpen) {
      if (this.props.resource.parents[0]) {
        if (this.props.resource.type === "email") {
          ...
          let extension = "";
          if (lastPartParts.length > 1) {
            saveAs = decodeURIComponent(
              lastPartParts.slice(0, lastPartParts.length - 1),
            );
            extension = lastPartParts[lastPartParts.length - 1];
  ...
  download(text) {
    ...
    if (this.props.resource.type === "email") {
      ...
      const emailToString = (emailPair) =>
        (emailPair.displayName || "") +
        (emailPair.displayName && emailPair.address ? " / " : "") +
        (emailPair.address || "");
      let header = `From:
...
`;
    ...
    element.setAttribute(
      "download",
      `${this.state.saveAs}.${this.state.extension}`,
    );
  ...
  onConfirmClick = (e?) => {
    ...
    const getDownloadType = (downloadType) =>
      downloadType.startsWith("transcript")
        ? "transcript"
    ...
    switch (getDownloadType(this.state.downloadType)) {
      case "preview": {
        authorizedDownload(
          authPreviewLink(this.props.resource.uri, filename),
        ).then((target) => (window.location.href = target));
        break;
      }
      case "extractedText": {
        const text = this.props.resource.text.contents
      ...
      case "transcript": {
        const text = this.props.resource.transcript[
          this.state.downloadLanguage
        ].contents
          .replace("<result-highlight>", "")
          .replace("</result-highlight>", "");
        this.download(text);
      ...
      case "ocrText": {
        this.download(
          this.props.resource.ocr[this.state.downloadLanguage].contents,
        );
        break;
      }
      case "original":
      default: {
        authorizedDownload(
          authDownloadLink(this.props.resource.uri, filename),
        ).then((target) => (window.location.href = target));
        break;
      }
    }
  ...
  downloadTypes() {
    let types: { value: string; label: string; language?: string }[] =
      this.props.resource.type !== "email"
    ...
    if (this.props.resource.transcript) {
      Object.keys(this.props.resource.transcript).forEach((language) => {
        types.push({
          value: `transcript${startCase(language)}`,
          label: `Transcript (${startCase(language)})`,
          language,
        });
      });
    }
    ...
    if (this.props.resource.ocr) {
      ...
      for (const language of languages) {
        types.push({
          value: "ocrText",
          label: `OCR Text (${startCase(language)})`,
          language,
        });
      }
    }
  ...
  downloadTypeSelected = (v) => {
    const getValue = (value) =>
      value.startsWith("transcript") ? "transcript" : value;
    ...
    switch (getValue(v.value)) {
      ...
      case "transcript":
        this.setState({
          downloadType: v.value,
          downloadLanguage: v.language,
          extension: `${v.language}.transcript.txt`,
        });
        break;
      case "ocrText":
        this.setState({
          downloadType: "ocrText",
          downloadLanguage: v.language,
          extension: `${v.language}.ocr.txt`,
        });
        break;
      default:
      case "original":
    ...
    return (
      <form className="form">
        ...
        <button
          ...
          disabled={!this.state.saveAs || !this.state.downloadType}
        >
          Download
        </button>
      </form>
