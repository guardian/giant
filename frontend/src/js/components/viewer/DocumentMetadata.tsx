...
export class DocumentMetadata extends React.Component<any, any> {
  static propTypes = {
  ...
  renderEnrichedMetadata = (key, value) => {
    const prettyValue =
      key === "createdAt" || key === "lastModified"
        ? hdate.prettyPrint(new Date(value), { showTime: true })
        : value;
          ...
          <li className="sidebar__list-item">
            ...
            <div className="sidebar__list-value">
              <a
                ...
                href={`http://localhost:9200/pfi/_doc/${this.props.resource.uri}`}
              >
                dev
              </a>
            </div>
            <div className="sidebar__list-value">
              <a
                target="_blank"
                rel="noopener noreferrer"
                href={`http://localhost:19200/pfi/_doc/${this.props.resource.uri}`}
              >
                prod
              </a>
            </div>
  ...
  render() {
    ...
    const emdKeys = this.props.resource.enrichedMetadata
      ? Object.keys(this.props.resource.enrichedMetadata)
      : undefined;
    const hasEnrichedMetadata = emdKeys && !!emdKeys.length;
    const enrichedMetadata = hasEnrichedMetadata
      ? emdKeys.map((k) => ({
          key: k,
          value: this.props.resource.enrichedMetadata[k],
        }))
      : [];
    ...
    return (
      <div className="sidebar__group">
        ...
        <div className="sidebar__title">
          ...
          {hasEnrichedMetadata ? (
            <button className="btn" onClick={this.toggleRaw}>
              {this.state.raw ? "View Metadata" : "View Raw Metadata"}
            </button>
          ) : (
            false
          )}
        ...
        <ul className="sidebar__list">
          {!this.state.raw && hasEnrichedMetadata
            ? enrichedMetadata.map((metadata) =>
                this.renderEnrichedMetadata(metadata.key, metadata.value),
              )
            : metadata.map((metadata) =>
                this.renderMetadata(metadata.key, metadata.value),
              )}
        </ul>
