...
function LoadingPreview(): React.ReactElement {
  return (
    <div className="preview__dialog">
      ...
      <p>
        <small>
          This will take a while the first time a document is viewed
        </small>
      </p>
    </div>
  );
...
export function Preview({ resource }: PreviewProps): React.ReactElement {
  ...
  const transcriptsAvailable =
    resource.vttTranscript && Object.keys(resource.vttTranscript).length > 0;
  ...
  if (doc) {
    ...
    if (mimeType && mimeType.startsWith("video/")) {
      if (transcriptsAvailable && resource.vttTranscript) {
        return (
          <TranscriptViewer
            transcripts={resource.vttTranscript}
            mediaUrl={doc}
            mediaType={"video"}
          />
        );
    ...
    if (mimeType && mimeType.startsWith("audio/")) {
      ...
      return (
        <audio
          className="viewer__preview-audio"
          src={doc}
          controls
          onError={() => onMediaError(mimeType)}
        />
