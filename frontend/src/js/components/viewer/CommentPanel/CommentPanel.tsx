...
export function groupCommentsByTop(
  comments: CommentData[],
  highlightRenderedPositions: HighlightRenderedPositions,
  commentHeights: { [commentId: string]: number },
  margin: number,
): CommentGroup[] {
  ...
  return sortBy(
    [...grouped.entries()].map(([top, comments]) => {
      return {
        top,
...
export function layoutComments(
  ...
  const root = groups.find(({ comments }) =>
    comments.some(({ id }) => id === rootCommentId),
  );
  ...
  groups.forEach((group, ix) => {
    ...
    if (root !== undefined && group.top === root.top) {
      ...
      const beforeRootInGroup = takeWhile(
        root.comments,
        ({ id }) => id !== rootCommentId,
      );
      const offsetWithinGroup =
        sumBy(beforeRootInGroup, "height") + margin * beforeRootInGroup.length;
...
export const CommentPanel: FC<CommentPanelProps> = ({
  ...
  const postCommentRefresh = (newComment: string) => {
    ...
    postComment(uri, newComment, anchor).then(() => {
  ...
  const margin = 10; // px
  const groups = groupCommentsByTop(
    comments,
    highlightRenderedPositions,
    commentHeights,
    margin,
  );
  const positions = layoutComments(
    groups,
    margin,
    focusedCommentId || previousFocusedCommentId,
  );
      ...
      {visibleComments
        ...
        .map((c, ix) => {
          ...
          return (
            <div
              key={c.id}
              className="comment comment--animatable"
              style={style}
              onClick={onCommentClick(c.id)}
              ref={onCommentMount(c.id)}
            >
              <Comment.Group>
                <Comment>
                  <Comment.Content>
                    ...
                    <Comment.Metadata>
                      <div>
                        {hdate.prettyPrint(new Date(c.postedAt), {
                          showTime: true,
                        })}
                      </div>
                    </Comment.Metadata>
