import { layoutComments, groupCommentsByTop, CommentGroup } from './CommentPanel';
import { CommentData } from '../../../types/Resource';
import { HighlightRenderedPositions } from '../TextPreview';

function comment({ id, postedAt }: { id: string, postedAt: number }): CommentData {
    return { id, postedAt, author: { username: 'test', displayName: 'test' }, text: 'test' };
}
...
describe('groupCommentsByTop', () => {
    test('only comments with a top and height are included', () => {
        ...
        const expected: CommentGroup[] = [
            ...
            { top: 5, height: 20, comments: [
                { id: '1', height: 10, postedAt: 1 }
            ]}
        ];

        expect(groupCommentsByTop(comments, tops, heights, 10)).toStrictEqual(expected);
    });
...
describe('layoutComments', () => {
    ...
    test('layout linearly', () => {
        ...
        const input: CommentGroup[] = [
            ...
            { top: 60, height: 20, comments: [
                { id: '3', height: 10, postedAt: 3 },
            ]},
        ];
    ...
    test('cascade pull up multiple groups above the selected comment', () => {
        const input: CommentGroup[] = [
            ...
            { top: 40, height: 10, comments: [
                { id: '3', height: 10, postedAt: 3 }
            ]},
            { top: 45, height: 20, comments: [
                { id: '4', height: 20, postedAt: 4 },
            ]},
        ];
