...
export default class ViewerActions extends React.Component<any, any> {
  static propTypes = {
    ...
    return (
      <div className="sidebar__list-item">
        <div className="btn-group btn-group--left">
          <button
            className="btn"
            onClick={() => this.setState({ addToWorkspaceModalOpen: true })}
          >
            Add to Workspace
          </button>
          <DownloadButton />
          ...
          {!this.props.disableDelete &&
            ...
            this.props.resource && (
              <DeleteButton
                deleteBlob={deleteBlobForAdmin}
                buttonTitle="Admin delete"
              />
            )}
        </div>
