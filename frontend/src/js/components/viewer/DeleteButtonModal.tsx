...
export function DeleteButtonModal({
  ...
  const tooltip = hasChildResources
    ? "Item cannot be deleted as it has child resources"
    : "Delete item";
  ...
  return (
    <React.Fragment>
      <Modal isOpen={modalOpen} isDismissable={false} dismiss={() => {}}>
        <div className="form form-full-width">
          <h2 className="modal__title">{modalTitle[deleteStatus]}</h2>
          <div className="form__row">{modalMessage[deleteStatus]}</div>
