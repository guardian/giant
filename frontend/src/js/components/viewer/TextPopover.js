...
class TextPopoverUnconnected extends React.Component {
    ...
    search = () => {
        history.push(buildLink('/search', this.props.urlParams, {q: JSON.stringify(['"' + window.getSelection().toString() + '"']), page: '1'}));
    }
            ...
            <SelectionPopover
                ...
                <div className='textpopover-wrapper'>
                    <div className='textpopover'>
                        <button className='btn textpopover__button' onClick={this.search}><SearchIcon className='textpopover-icon'/></button>
                        <button className='btn' onClick={this.addComment}><CommentIcon className='textpopover-icon'/></button>
                    </div>
                </div>
            </SelectionPopover>
        </React.Fragment>;
...
export default connect(mapStateToProps, mapDispatchToProps)(TextPopoverUnconnected);
