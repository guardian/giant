...
export function renderIcon(resource: BasicResource) {
    switch (resource.type) {
        case 'directory': return <FolderIcon className='file-browser__icon'/>;
...
export default function PagedBrowser({ resource }: { resource: Resource }) {
    const totalPages = Math.floor(resource.children.length / MAX_NUMBER_OF_CHILDREN);
    const [page, setPage] = useState(1);

    // Pull folders up to the top
    const sortedChildren = sortBy(resource.children, ({ type }) => type !== 'directory');
    const childrenToDisplay = sortedChildren.slice((page - 1) * MAX_NUMBER_OF_CHILDREN, ((page - 1) * MAX_NUMBER_OF_CHILDREN) + MAX_NUMBER_OF_CHILDREN);
        ...
        <div className='file-browser__wrapper'>
            <table className='file-browser'>
                <tbody>
                    {childrenToDisplay.map(resource => {
                        ...
                        return <tr key={resource.uri} className="file-browser__entry">
                            <td>
                                {renderIcon(resource)}
            ...
            <Pagination
                ...
                onPageChange={(e, { activePage }) => {
                    if(activePage) {
                        setPage(typeof activePage === 'number' ? activePage : parseInt(activePage));
                    }
                }}
            />
        </div>
