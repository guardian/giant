...
class AddToWorkspaceModalUnconnected extends React.Component<Props, State> {
  ...
  onSubmit = (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    if (this.props.currentWorkspace) {
      const parentNode =
        this.state.selectedParentNode ?? this.props.currentWorkspace.rootNode;
  ...
  workspaceSelection = () => {
    return this.props.workspacesMetadata.map((w) => ({
      value: w.id,
      label: w.name,
    }));
  };
    ...
    return (
      <div className="workspace__tree workspace__tree--fixed">
        <TreeBrowser
          ...
          selectedEntries={
            this.state.selectedParentNode ? [this.state.selectedParentNode] : []
          }
          focusedEntry={
            this.state.selectedParentNode ? this.state.selectedParentNode : null
          }
            ...
            columns: [
              {
                ...
                align: "left",
                render: (entry) => (
                  <React.Fragment>{entry.name || "--"}</React.Fragment>
                ),
    ...
    if (this.props.currentWorkspace) {
      const prefix = this.state.selectedParentNode
        ? displayRelativePath(
            ...
            this.state.selectedParentNode.id,
          )
        ...
        <form className="form" onSubmit={this.onSubmit}>
          ...
          <div className="form__row">
            <button
              type="submit"
              className="btn"
              disabled={!this.state.saveAs || !this.props.currentWorkspace}
            >
              Save
            </button>
          </div>
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(AddToWorkspaceModalUnconnected);
