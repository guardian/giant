...
class Login extends React.Component<any, any> {
  ...
  componentDidMount() {
    ...
    if (
      this.props.genesisSetupComplete &&
      this.props.config.userProvider === "panda"
    ) {
      this.autoLogin();
    }
  }
  ...
  autoLogin = () => {
    ...
    localStorage.setItem(Login.AUTO_AUTH_KEY, String(attempts));
  ...
  databaseLoginEnabled() {
    return (
      this.props.config.userProvider === "database" &&
      ((!this.props.require2fa && this.state.username && this.state.password) ||
        (this.props.require2fa && this.state.tfaCode))
    );
  }
  ...
  renderActions = () => {
    let progress = <p className="form__error">{this.props.errors.join(" ")}</p>;
    ...
    const enabled =
      (this.databaseLoginEnabled() || this.pandaLoginEnabled()) &&
    ...
    return (
      <div className="app__page app__page--centered">
        <form className="form" onSubmit={this.login}>
          <h2 className="form__title">Login</h2>
          <div>
            <label className="form__label" htmlFor="#tfa">
              Authentication Code
            </label>
            <input
              ref={(input) => {
    ...
    return (
      <div className="app__page app__page--centered">
        <div className="form">
          ...
          <div>
            <p>
              Your account has been blocked from accessing the platform for the
              following reasons:
            </p>
            ...
            <p>
              ...
              providing them with the information above.
            </p>
          </div>
        </div>
      </div>
  ...
  renderPanDomainProvider() {
    if (this.props.requirePanda) {
      ...
      const returnUrl =
        currentUrl.searchParams.get("returnUrl") || currentUrl.href;
