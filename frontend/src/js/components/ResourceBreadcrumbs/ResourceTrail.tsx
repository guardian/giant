    ...
    case "email":
      return (
        <PathBasedResource
          ...
          lastSegmentOnly={props.lastSegmentOnly}
        />
      );
    default:
...
export function buildSegments(resource: BasicResource): PathSegment[] {
  ...
  const parts =
    uriParts.length === displayParts.length
...
function PathBasedResource({
  className,
  resource,
  lastSegmentOnly,
}: PathBasedResourceProps) {
  const allSegments = buildSegments(resource);
  const segments = lastSegmentOnly
