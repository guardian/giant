...
export default class Timeline extends React.Component {
    ...
    return (
      <div>
        {this.props.data.thread ? (
          <ReactTooltip effect="solid" insecure={false} html={true} />
        ) : (
          false
        )}
        {this.renderTimeline()}
  ...
  renderTimeline() {
    ...
    const anyUnknowns = data.thread.some(
      (v) =>
        !_.get(v, "email.haveSource") ||
        (_.get(v, "email.metadata.fromName") === undefined &&
          _.get(v, "email.metadata.fromAddress") === undefined),
    );
    // Flag so we can skip the timestamps
    const anyTimestamps = data.thread.some((v) =>
      _.get(v, "email.metadata.sentAt"),
    );

    // You can end up where you've got no recipient so the above map won't add an unknown address
    if (
      anyUnknowns &&
      !knownRecipients.some((r) =>
        R.equals(r, { address: undefined, name: undefined }),
      )
    ) {
    ...
    return (
      <div className="timeline__container">
        <div className="timeline__chart">{chart ? chart : false}</div>
  ...
  renderAddresses(knownRecipients, userAxis, anyUnknowns, anyTimestamps) {
    ...
    const addresses = knownRecipients.map((r) =>
      r.name ? r.name : r.address ? r.address : "<Unknown>",
    );
    ...
    const names = svgDoc.append("g").selectAll("text").data(addresses).enter();
    // shadow
  ...
  renderChart(data, timelineAxis, userAxis, anyTimestamps) {
    ...
    const relationships = data.thread
      .map((t) =>
        t.neighbours.map((n) => {
    ...
    const relationshipSelected = (rel) =>
      this.props.selected === rel.from || this.props.selected === rel.to;
    ...
    drawRelationships(
      (rel) => rel.relation === "REFERENCED" && !relationshipSelected(rel),
      "timeline__reference",
    );
    drawRelationships(
      (rel) => rel.relation === "IN_REPLY_TO" && !relationshipSelected(rel),
      "timeline__reply",
    );
    ...
    drawRelationships(
      (rel) => rel.relation === "REFERENCED" && relationshipSelected(rel),
      "timeline__reference timeline__reference--selected",
    );
    drawRelationships(
      (rel) => rel.relation === "IN_REPLY_TO" && relationshipSelected(rel),
      "timeline__reply timeline__reply--selected",
    );
    ...
    svgDoc
      ...
      .on("click", (thread) => {
        if (thread.email.haveSource) {
          this.select(thread.email.uri);
        }
      });
    ...
    const axis = d3
      ...
      .tickValues(
        timelineAxis.domain().filter((uri) => {
    ...
    svgDoc
      ...
      .attr(
        "transform",
        `translate(${this.props.addressPadding},${chartHeight})`,
      )
      .call(axis);

    // add timestamps
    const threadsWithTimestamps = data.thread.filter((t) =>
      _.get(t, "email.metadata.sentAt"),
    );
    if (threadsWithTimestamps.length > 0) {
      const textElems = svgDoc
        .append("g")
        .selectAll("text")
