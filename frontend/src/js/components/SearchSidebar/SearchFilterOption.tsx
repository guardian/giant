...
export class SearchFilterOption extends React.Component<any, any> {
  static propTypes = {
  ...
  toggleMainSearchFilter = (searchFilterValue) => {
    ...
    if (!this.isMainOptionSelected()) {
      ...
      const filteredSelections = this.props.selectedOptions.filter(
        (selectOption) => {
      ...
      this.props.updateSelectedOptions(
        filteredSelections.concat([searchFilterValue]),
      );
    } else {
      this.props.updateSelectedOptions(
        this.props.selectedOptions.filter((a) => a !== searchFilterValue),
      );
    }
  };
  ...
  toggleSubOptionSearchFilter = (searchFilterValue) => {
    ...
    if (this.isMainOptionSelected()) {
      ...
      const subOptions = this.props.option.suboptions || [];
      const subOptionValues = subOptions.map((o) => o.value);
      const filteredSubOptions = subOptionValues.filter(
        (o) => o !== searchFilterValue,
      );

      this.props.updateSelectedOptions(
        oldSelections.concat(filteredSubOptions),
      );
      return;
    }

    if (this.props.selectedOptions.indexOf(searchFilterValue) === -1) {
      const newSelectedOptions = this.props.selectedOptions.concat([
        searchFilterValue,
      ]);
      ...
      const notAllOptionsAreSelected = allSubOptionsValues.reduce(
        (missingSoFar, value) => {
      ...
      if (notAllOptionsAreSelected) {
        ...
        //Add the main option
        this.props.updateSelectedOptions(
          filteredSelectedOptions.concat([this.props.option.value]),
        );
      }
    } else {
      this.props.updateSelectedOptions(
    ...
    return this.props.option.suboptions.map((subOption) => {
      return (
        <SearchFilterValue
          ...
          aggBucket={
            this.props.aggBucket
              ? this.props.aggBucket.buckets.find(
                  (b) => b.key === subOption.value,
                )
              : undefined
          }
      ...
      <div>
        <div className="sidebar__filtervalue" onClick={this.toggleExpanded}>
          <SearchFilterValue
            ...
            hideable={this.props.hideable}
            expandable={
              this.props.option.suboptions &&
              this.props.option.suboptions.length
            }
            expanded={this.state.expanded}
            selected={this.isMainOptionSelected()}
            toggleSelected={this.toggleMainSearchFilter}
            indeterminate={
              !this.isMainOptionSelected() && this.hasCheckedSubOptions()
            }
