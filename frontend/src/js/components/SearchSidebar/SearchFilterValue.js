...
export class SearchFilterValueUntranslated extends React.Component {
    ...
    isActiveFilter = () => {
        const activeFilters = _get(this.props, 'activeFilters.' + this.props.rootKey);
        if (activeFilters) {
            return activeFilters.some(filter => filter.startsWith(this.props.optionValue.value));
        } else {
            return false;
        }
    }
    ...
    renderAggCounts = () => {
        if (this.props.aggBucket) {
            return <div className='sidebar__count'>
                {this.props.aggBucket.count}
            </div>;
        }

        if (!this.props.aggBucket && this.props.currentResults) {
            return <div className='sidebar__count'>
                {this.props.missingAggValue}
            </div>;
        }
    ...
    render() {
        ...
        if (this.props.hideable && this.props.currentResults && !this.isActiveFilter() && this.props.aggBucket === undefined) {
            return false;
        }

        return (
            <div className={this.props.disabled ? 'sidebar__item sidebar__item--disabled' : 'sidebar__item'} onClick={this.onClick}>
                <div className='sidebar__chevron-container'>
                    {this.props.expandable ? <MenuChevron expanded={this.props.expanded || false} /> : false}
                </div>
                <div className='sidebar__item__text'>
                    {this.props.optionValue.display ? this.props.optionValue.display : this.props.optionValue.value}
                    {explanation
                        ? <InfoIcon className='info-icon' data-tip={markdownToHtml(explanation)} data-effect='solid'/>
                        : false
                    }
                </div>
