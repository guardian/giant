...
export class CollectionsSidebar extends React.Component {
    ...
    render() {
        ...
        const visibleCollections = this.props.collections.filter(({ users }) => users.includes(this.props.username));
            ...
            <div className='sidebar'>
                <div className='sidebar__group'>
                    ...
                    {visibleCollections.map((collection) => (
                        <CollectionItem key={collection.uri} uri={collection.uri} display={collection.display}/>
                    ))}
                </div>
            </div>
        );
...
function mapDispatchToProps(dispatch) {
    return {
        collectionsActions: bindActionCreators(Object.assign({}, getCollections), dispatch)
    };
}
