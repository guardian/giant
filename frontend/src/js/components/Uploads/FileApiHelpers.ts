...
export interface FileSystemFileEntry extends FileSystemEntry {
    fullPath?: string,
    file(successCallback: (file: File) => void, errorCallback?: (error: any) => void): undefined
}

export interface FileSystemDirectoryReader extends FileSystemEntry {
    readEntries(successCallback: (entries: FileSystemEntry[]) => void, errorCallback?: (error: any) => void): undefined
}
...
export async function readDirectoryEntry(entry: FileSystemDirectoryEntry): Promise<Map<string, File>> {
    async function entriesToMap(entries: FileSystemEntry[]): Promise<Map<string, File>> {
    ...
    return new Promise((resolve, reject) => {
        entry.createReader().readEntries(
            (entries) => entriesToMap(entries).then(resolve),
            reject
        );
    });
}