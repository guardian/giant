...
import { readFileEntry, readDirectoryEntry, FileSystemEntry, FileSystemFileEntry, FileSystemDirectoryEntry } from './FileApiHelpers';
...
function UploadButton({ action, loading, disabled, onClick }: { action: Action, loading: boolean, disabled: boolean, onClick: (action: Action) => void }) {
    // type=button prevents the button auto-submitting the form it is a part of
    return <Button primary type="button" disabled={disabled} loading={loading} onClick={() => onClick(action)}>
    ...
    render() {
        return <React.Fragment>
            ...
            <Button.Group onDragOver={this.onDragOver} onDrop={this.onDrop}>
                <UploadButton action='files' disabled={this.props.disabled} loading={this.state.readingFiles} onClick={this.onClick} />
                <Button.Or />
                <UploadButton action='directory' disabled={this.props.disabled} loading={this.state.readingFiles} onClick={this.onClick} />
            </Button.Group>
        </React.Fragment>;
    }
}
