...
export function isFailedFileUploadState(
  uploadState: FileUploadState,
): uploadState is FailedFileUploadState {
...
async function createIntermediateDirectories(
  ...
  for (const part of directories) {
    ...
    if (!idFromCache) {
      const { id } = (await addFolderToWorkspace(
        workspaceId,
        parent,
        part,
      )) as { id: string };
...
async function buildWorkspaceUploadMetadata(
  path: string,
  target: WorkspaceTarget,
  mutableFolderCache: Map<string, string>,
): Promise<WorkspaceUploadMetadata> {
  ...
  const parentNodeId = await createIntermediateDirectories(
    workspaceId,
    parentId,
...
async function uploadFiles(
  ...
  async function nextFile(
    ...
    if (file.size > MAX_FILE_UPLOAD_SIZE_BYTES) {
      console.error(
        `Error uploading ${path}: file is too large (limit ${MAX_FILE_UPLOAD_SIZE_MBYTES}MB`,
      );
      dispatch({
        ...
        state: {
          description: "failed",
          failureReason: `File too large (limit ${MAX_FILE_UPLOAD_SIZE_MBYTES}MB)`,
        },
      });
    } else {
      try {
        ...
        const metadata = await buildWorkspaceUploadMetadata(
          path,
          target,
          workspaceFolderCache,
        );
        await uploadFileWithNewIngestion(
          target.collectionUri,
          target.ingestionName,
          uploadId,
          file,
          path,
          metadata,
...
function getCurrentlyUploading(
  state: State,
): { file: string; uploadState: InProgressFileUploadState } | undefined {
...
function FileUploadProgressBar({
  file,
  uploadState: { loadedBytes, totalBytes },
}: {
  file: string;
  uploadState: InProgressFileUploadState;
}) {
  if (loadedBytes && loadedBytes !== totalBytes) {
    return (
      <Progress value={loadedBytes} total={totalBytes}>
        Uploading {file}:{" "}
        {`${filesize(loadedBytes)} of ${filesize(totalBytes)}`}
      </Progress>
    );
  } else {
    /* Indicate activity (via a nice sparkly animation) */
    return (
      <Progress active value={1} total={1}>
        Processing {file}
      </Progress>
    );
  }
...
export default function UploadFiles(props: Props) {
  ...
  const isComplete = [...state.files.values()].some(
    ({ state }) => state.description === "uploaded",
  );
    ...
    if (props.focusedWorkspaceEntry) {
      ...
      if (isTreeNode(focusedWorkspaceEntry)) {
        ...
        if (parentId && parentId !== rootNodeId) {
          const focusedUploadFolder =
            props.expandedNodes &&
            props.expandedNodes.find((node) => node.id === parentId);
  ...
  const focusedWorkspaceRelativePath =
    focusedWorkspaceFolder && props.workspace.rootNode
