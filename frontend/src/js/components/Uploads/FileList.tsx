...
function buildTree(files: string[], collapsed: string[]): string[] {
  ...
  return Array.from(ret).filter(
    (path) =>
      !collapsed.some((prefix) => path.startsWith(prefix) && path !== prefix),
  );
}
...
function StatusIcon({
  ...
  const allUnderPath = Array.from(files.entries()).filter(([p]) =>
    p.startsWith(path),
  );

  const anyFailed = allUnderPath.some(
    ([, { state }]) => state.description === "failed",
  );
  if (anyFailed) {
    return <Icon name="exclamation triangle" size="tiny" color="red" />;
  }

  const anyUploading = allUnderPath.some(
    ([, { state }]) => state.description === "uploading",
  );
  if (anyUploading) {
    return <Loader active inline size="tiny" />;
  }

  const allUploaded = allUnderPath.every(
...
export default function FileList({
  ...
  const failedFiles = [...files.values()].filter((f) =>
    isFailedFileUploadState(f.state),
  );
  const anyFailed = failedFiles.length > 0;
  const failureReasons = failedFiles
    .map((s) => ({
      file: s.file.name,
      failureReason: isFailedFileUploadState(s.state)
        ? s.state.failureReason
        : undefined,
    }))
        ...
        <table className="file-browser">
          <tbody>
            {filesAndFolders.map((path) => {
              ...
              return (
                <tr className="file-browser__entry" key={path}>
                  <td className="file-browser__cell">
                    ...
                    {isLeaf ? (
                      <MenuChevron
                        onClick={() =>
                          setUserCollapsed(
                            updateCollapsed(path, !isCollapsed, collapsed),
                          )
                        }
