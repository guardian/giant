...
export const CaptureFromUrl = connect(
  ...
  useEffect(() => {
    ...
    const navigateToFindParentOfLeaf = (
      leaf: TreeLeaf<WorkspaceEntry>,
      node: TreeNode<WorkspaceEntry>,
    ): TreeNode<WorkspaceEntry> | null => {
    ...
    if (focusedEntry && isTreeLeaf(focusedEntry) && currentWorkspace) {
      setFocusedEntry(
        navigateToFindParentOfLeaf(focusedEntry, currentWorkspace.rootNode)!,
      );
    }
  ...
  const parentFolder =
    selectedEntry && isTreeNode(selectedEntry) ? selectedEntry : null;

  const isTargetFolderNameConflict = !!parentFolder?.children?.some(
    (_) => _.name === saveAs,
  );
      ...
      <Modal isOpen={isOpen} dismiss={() => setIsOpen(false)}>
        <form className="form" onSubmit={onSubmit}>
          ...
          <EuiCallOut
            ...
            size="s"
          >
            For now, this feature will attempt to capture video, text and a
            screenshot from a URL (e.g. youtube/insta url).
            <br />
            Please share any feedback with the team -
          ...
          <div className="form__row">
            ...
            <Select
              ...
              onChange={(selected) =>
                selected && setCurrentWorkspace(selected.value)
              }
            />
          </div>
            ...
            currentWorkspace.rootNode.children.length > 0 && (
              <div className="form__row">
                ...
                <div
                  ...
                  style={{ maxHeight: "calc(100vh - 590px)" }}
                >
                  <TreeBrowser
                    ...
                    onFocus={(entry) =>
                      isTreeNode(entry) && setFocusedEntry(entry)
                    }
          ...
          <div className="form__row">
            ...
            <em>
              Capturing a URL into Giant will capture as much as possible from
              the page, so a new folder is required to keep everything together.
            </em>
            <input
              type="text"
              value={saveAs}
            ...
            {isTargetFolderNameConflict && (
              <div className="error-bar__warning" style={{ padding: "5px" }}>
                A file or folder with this name already exists in the selected
                folder.
              </div>
            )}
          </div>
          ...
          <div className="form__row">
            ...
            {currentWorkspace && parentFolder && saveAs && (
              <>
                {" "}
                to{" "}
                <code>{`${displayRelativePath(currentWorkspace.rootNode, parentFolder.id)}${saveAs}`}</code>
              </>
            )}
          </div>
        </form>
