...
test("first parent name for blob", () => {
    const input = { uri: "1234", parents: [{ uri: "collection/ingestion/test.jpg" }] };
...
test("collapse query parts", () => {
    // This happens when you manually deleted everything from the search bar
    expect(calculateSearchTitle({ q: JSON.stringify(["hello", "world"]) }))
        .toBe("hello world - Search - Giant");
});
...
test("collapse chips in query parts", () => {
    ...
    // This happens when you manually deleted everything from the search bar
    expect(calculateSearchTitle({ q: JSON.stringify(q) }))
        .toBe("hello Mime Type: application/json - Search - Giant");
});
