...
export default class PageNavigator extends React.Component {
    ...
    renderPageButton(number) {
        if (number === this.props.currentPage) {
            return (<span key={'page_' + number} className='page-navigator__page page-navigator__page--selected'>{number}</span>);
        }
        return (<button key={'page_' + number} className='btn-link page-navigator__page' onClick={() => this.props.pageSelectCallback(number)}>{number}</button>);
    }
    ...
    render() {
        const firstDisplayedPage = Math.max(2, this.props.currentPage - this.props.pageSpan);
        const lastDisplayedPage = Math.min(this.props.lastPage - 1, firstDisplayedPage + this.props.pageSpan * 2);
