...
export default class InputSupper extends React.Component<any, any> {
  currentRef: any;
  deleteExpiryInterval: any;
  static propTypes = {
  ...
  chippifyText(chips, value) {
    let overrideElements = JSON.parse(value).map((e: any) => {
      if (_isString(e)) {
        return {
          type: "input",
          value: e,
    ...
    if (
      this.state.elements.length === overrideElements.length &&
      this.state.elements.every((e, i) =>
        overrideElements[i].value.startsWith(e.value),
      )
    ) {
      this.setState({
        elements: this.state.elements.map((e, i) =>
          Object.assign(e, overrideElements[i]),
        ),
      });
    } else {
      this.setState({
  ...
  chipSelected = (field) => {
    ...
    if (element.type === "input") {
      const { value, chipStart, cursorPosition } =
        this.state.elements[this.state.currentElement];
  ...
  onNegateClicked = (index) => {
    ...
    const newElement = Object.assign({}, this.state.elements[index], {
  ...
  onDeleteClicked = (index) => {
    ...
    const newElements = removed.reduce((acc: any[], cur: any, i: number) => {
      if (
        i - 1 >= 0 &&
        cur.type === "input" &&
        acc[acc.length - 1].type === "input"
      ) {
  ...
  onEnter = () => {
    const selectedField =
      this.state.filteredSuggestions[this.state.currentChip];
    if (selectedField) {
    ...
    switch (e.type) {
      case "input": {
        ...
        const shouldRenderSuggestions =
          ...
          hasFocus;
        const renderSuggestions = shouldRenderSuggestions
        ...
        return (
          <InlineInput
            ...
            ref={(r) =>
              this.state.currentElement === index
                ? (this.currentRef = r)
                : undefined
            }
            index={index}
