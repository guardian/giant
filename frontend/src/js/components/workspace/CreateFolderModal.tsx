...
export default class CreateFolderModal extends React.Component<Props, State> {
    ...
    onSubmit = (e: React.SyntheticEvent<HTMLFormElement>) => {
        ...
        const path = findPath(this.props.parentEntry.id, [], this.props.workspace.rootNode);
        if (!path) {
            return;
        }
        ...
        this.props.addFolderToWorkspace(this.props.workspace.id, parentId, this.state.name);
    ...
    render() {
        const relativePath = displayRelativePath(this.props.workspace.rootNode, this.props.parentEntry.id);
