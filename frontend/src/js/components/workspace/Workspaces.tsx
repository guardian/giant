...
class WorkspacesUnconnected extends React.Component<Props, State> {
  ...
  renderIcon = (entry: TreeEntry<WorkspaceEntry>) => {
    if (isTreeLeaf<WorkspaceEntry>(entry) && isWorkspaceLeaf(entry.data)) {
      switch (entry.data.processingStage.type) {
        case "processing":
          return (
            <Loader active inline size="tiny" className="file-browser__icon" />
          );
        case "failed":
          return (
            <Icon
  ...
  renderReprocessIcon = (entry: TreeEntry<WorkspaceEntry>) => {
    ...
    const buttonStyle = {
      paddingLeft: "0.4em",
      color:
        this.state.hoverOverReprocessIconEntry === entry ? "black" : "grey",
    };
    return (
      <Popup
        content="Reprocess source file"
        trigger={
    ...
    if (isTreeLeaf<WorkspaceEntry>(entry) && isWorkspaceLeaf(entry.data)) {
      return (
        <React.Fragment>
          ...
          {entry.data.processingStage.type === "failed" &&
            this.renderReprocessIcon(entry)}
        </React.Fragment>
      );
      ...
      return (
        <>
          ...
          {entry.data.descendantsProcessingTaskCount > 0 &&
            entry.data.descendantsFailedCount > 0 && <>&nbsp;&amp;&nbsp;</>}
          {entry.data.descendantsFailedCount > 0 && (
            <em>{entry.data.descendantsFailedCount.toLocaleString()} failed</em>
          )}
        </>
      );
    }
  ...
  allColumns = [
    ...
    },
    {
      ...
      sort: (a: TreeEntry<WorkspaceEntry>, b: TreeEntry<WorkspaceEntry>) =>
        this.stringSort(a.data.addedBy.displayName, b.data.addedBy.displayName),
    },
    {
      name: "Added On",
      align: "left" as const,
      ...
      render: (entry: TreeEntry<WorkspaceEntry>) => (
        <React.Fragment>
          {hdate.prettyPrint(new Date(Number(entry.data.addedOn)), {
            showTime: true,
          })}
        </React.Fragment>
      ),
      ...
      sort: (a: TreeEntry<WorkspaceEntry>, b: TreeEntry<WorkspaceEntry>) =>
        this.numberSort(a.data.addedOn || 0, b.data.addedOn || 0),
    },
    {
      name: "File Type",
      ...
      render: (entry: TreeEntry<WorkspaceEntry>) => (
        <React.Fragment>
          {isTreeLeaf<WorkspaceEntry>(entry) && isWorkspaceLeaf(entry.data)
            ? entry.data.mimeType
            : ""}
        </React.Fragment>
      ),
    ...
    },
    {
      ...
      render: (entry: TreeEntry<WorkspaceEntry>) => this.renderStatus(entry),
      sort: (a: TreeEntry<WorkspaceEntry>, b: TreeEntry<WorkspaceEntry>) => {
        // Sort folders on top (or bottom)
        let aVal = "";
        let bVal = "";
  ...
  componentDidUpdate(prevProps: Props, prevState: State) {
    this.setTitle();
    if (
      localStorage.getItem("selectedWorkspaceId") !== this.props.match.params.id
    ) {
    ...
    if (
      workspaceLocation &&
      !this.state.haveAppliedWorkspaceLocationParam &&
  ...
  setSelectedEntriesAfterShiftClick = (
    ...
    this.setState({
      previousShiftClickSelectedEntries: newShiftClickSelectedEntries,
    });
  };
  ...
  onDeleteModalOpen = (isOpen: boolean) => {
    if (isOpen)
      ...
      this.setState({
        deleteModalContext: { entry: null, isOpen, status: "unconfirmed" },
      });
  };
  ...
  onRemoveCompleteHandler = (isSuccess: boolean) => {
    ...
    if (modalContext.isOpen) {
      const status = isSuccess ? "done" : "failed";
      this.setState({
        removeFromWorkspaceModalContext: { ...modalContext, status },
      });
    }
  };

  onDeleteItem =
    (workspaceId: string, entry: TreeEntry<WorkspaceEntry> | null) => () => {
      if (entry && isWorkspaceLeaf(entry.data)) {
        const deleteContext = this.state.deleteModalContext;
        this.setState({
          deleteModalContext: { ...deleteContext, status: "doing" },
        });
  ...
  onRemoveFromWorkspace =
    (workspaceId: string, entry: TreeEntry<WorkspaceEntry> | null) => () => {
      if (entry) {
        const removeFromWorkspaceModalContext =
          this.state.removeFromWorkspaceModalContext;
        this.setState({
          removeFromWorkspaceModalContext: {
            ...removeFromWorkspaceModalContext,
            status: "doing",
  ...
  renderContextMenu(
    ...
    const copyFilenameContent = isFolder
      ? "Copy folder name"
      : "Copy file name";
    const copyFilePathContent = isFolder
      ? "Copy folder path"
      : "Copy file path";
    const copyCaptureURLContent = "Copy URL this was captured from";
    const copyLinkToFolderContent = "Copy link to folder";
    ...
    const items = [
      {
        key: "copyFilename",
        content: copyFilenameContent,
        icon: isFolder ? "folder outline" : "copy",
      },
      {
        key: "copyFilePath",
        content: copyFilePathContent,
        icon: isFolder ? "folder outline" : "copy",
      },
    ];
    ...
    if (isWorkspaceNode(entry.data) && !isRemoteIngest) {
      items.push({
        key: "copyLinkToFolder",
        content: copyLinkToFolderContent,
        icon: "linkify",
      });
      items.push({
        key: "search",
    ...
    return (
      <DetectClickOutside onClickOutside={this.closeContextMenu}>
        <Menu
            ...
            if (
              ...
              switch (menuItemProps.content) {
                ...
                case copyLinkToFolderContent:
                  const folderUrl = `${window.location.origin}/workspaces/${workspace.id}/${entry.id}`;
  ...
  renderFolderTree = (workspace: Workspace) => {
    ...
    const createFolderDestination =
      this.props.focusedEntry || workspace.rootNode;
    const selectedCount = this.props.selectedEntries.length;

    const selectedTotalCounts =
      selectedCount > 1 &&
      this.props.selectedEntries.reduce(
        (acc, entry) => {
          if (isWorkspaceNode(entry.data)) {
            return {
              descendantsNodeCount:
                acc.descendantsNodeCount +
                1 +
                (entry.data.descendantsNodeCount || 0),
              descendantsLeafCount:
                acc.descendantsLeafCount +
                (entry.data.descendantsLeafCount || 0),
            };
          } else {
            return {
      ...
      <>
        <div className="workspace__tree-header">
          <div className="workspace__tree-actions">
            <button
              className="btn btn--primary workspace__button"
            ...
            >
              <CreateFolderModal
                onComplete={this.dismissModal}
                workspace={workspace}
                parentEntry={createFolderDestination}
                addFolderToWorkspace={this.props.addFolderToWorkspace}
              />
            </Modal>

            {selectedTotalCounts && (
              <EuiText size="xs" style={{ lineHeight: "0.9rem" }}>
                {this.props.selectedEntries.length} items selected
                <br />
                <FileAndFolderCounts
                  ...
                  descendantsLeafCount={
                    selectedTotalCounts.descendantsLeafCount
                  }
                />
              </EuiText>
            )}
          ...
          <div className="workspace__tree-actions" style={{ gap: "2px" }}>
            <EuiText
              size="xs"
              style={{
                visibility: this.props.isGettingWorkspace
                  ? "visible"
                  : "hidden",
              }}
            >
            ...
            <EuiText size="xs" style={{ marginRight: "10px" }}>
              last refreshed{" "}
              <FromNowDurationText
                date={this.props.currentWorkspaceLastRefreshedAt}
              />
            </EuiText>
          </div>
        </div>
...
function mapStateToProps(state: GiantState) {
  return {
    ...
    currentWorkspaceLastRefreshedAt:
      state.workspaces.currentWorkspaceLastRefreshedAt,
