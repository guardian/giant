...
class WorkspacesSidebarUnconnected extends React.Component<Props, State> {
  ...
  UNSAFE_componentWillReceiveProps(props: Props) {
    if (props.workspacesMetadata !== this.props.workspacesMetadata) {
      ...
      const selectedWorkspaceExists =
        props.workspacesMetadata &&
        !!props.workspacesMetadata.find(
      ...
      this.setState({
        myWorkspaces: sortBy(
          workspacesMetadata.filter(
            (w) => !w.isPublic && w.owner.username === currentUser.username,
          ),
          (w) => w.name.toLowerCase(),
        ),
        sharedWorkspaces: sortBy(
          workspacesMetadata.filter(
            (w) => !w.isPublic && w.owner.username !== currentUser.username,
          ),
          (w) => w.name.toLowerCase(),
        ),
        publicWorkspaces: sortBy(
          workspacesMetadata.filter((w) => w.isPublic),
          (w) => w.name.toLowerCase(),
        ),
      });
    }
  ...
  renderSidebarItems = (workspaces: WorkspaceMetadata[]) =>
    workspaces.map((w) => (
  ...
  render() {
    ...
    const noWorkspacesMessage = (
      <div className="sidebar__item">
        <div className="sidebar__item__text sidebar__item__text--disabled">
          no workspaces
        </div>
      </div>
    );
      ...
      <div className="sidebar">
        <div className="sidebar__group">
          ...
          {this.state.myWorkspaces.length !== 0
            ...
            : noWorkspacesMessage}
          <button
            className="btn btn--primary sidebar__button"
