...
export function DeleteModal({
  ...
  if (entry !== null) {
    ...
    const modalMessage: Record<ModalStatus, string> = {
      unconfirmed: `This will ${
        isRemoteIngest ? "dismiss the failed capture" : "delete the file"
...
export function RemoveFromWorkspaceModal({
  removeHandler,
  isOpen,
  setModalOpen,
  removeStatus,
    ...
    <React.Fragment>
      <Modal isOpen={isOpen} isDismissable={true} dismiss={onDismiss}>
        <div className="form form-full-width">
          <h2 className="modal__title">{modalTitle[status]}</h2>
          <div className="form__row">{modalMessage[status]}</div>
