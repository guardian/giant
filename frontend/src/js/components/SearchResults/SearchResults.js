...
export default class SearchResults extends React.Component {
    ...
    shouldComponentUpdate(newProps) {
        return this.props.isSearchInProgress !== newProps.isSearchInProgress || this.props.compact !== newProps.compact;
    }
    ...
    componentDidMount() {
        ...
        if(previousResult) {
            ...
            } catch(e) { // Support for editorial version of firefox
                previousResult.scrollIntoView({
                    behaviour: 'smooth',
                    block: 'start'
                });
    ...
    render () {
        if (this.props.searchResults) {
            const hasHistogram = this.props.searchResults.aggs.length > 0 &&
                this.props.searchResults.aggs.find(a => a.key === 'createdAt').buckets.length > 0;
            ...
            if (this.props.compact) {
                return (
                    <div ref={div => {this.wrapper = div;}} className={resultsClass}>
                        <CompactResultsTable lastUri={this.props.lastUri} searchResults={this.props.searchResults}/>
                    </div>
                );
            } else {
                return (
                    <div ref={div => {this.wrapper = div;}} className={resultsClass}>
                        {this.props.searchResults.results.map((result, index) => <SearchResult key={result.uri} lastUri={this.props.lastUri} index={index} searchResult={result} />)}
                    </div>
                );
            }
        } else {
