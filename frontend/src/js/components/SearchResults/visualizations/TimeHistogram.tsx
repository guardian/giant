...
export default class TimeHistogram extends React.Component<any, any> {
  static propTypes = {
  ...
  render() {
    const createdAt = this.props.results.aggs.find(
      (a) => a.key === "createdAt",
    );
    ...
    const queryByDate = (datum) => {
      const qAsArray =
        this.props.q !== '["*"]' ? JSON.parse(this.props.q) : [""];
      // Filter existing date chips from the current query string and any double spaces that result from thsi removal
      const qWithoutDateChips = qAsArray.filter(
        (chip) => chip.n !== "Created Before" && chip.n !== "Created After",
      );
      const cleanedQWithoutDateChips = qWithoutDateChips.filter(
        (chip, index) => !(chip === "" && qAsArray[index - 1] === ""),
      );
      ...
      const query = JSON.stringify(
        cleanedQWithoutDateChips.concat([beforeChip, "", afterChip, ""]),
      );
    ...
    return (
      <div style={{ width: "100%", height: "200px" }}>
        <Chart size={{ height: 200 }}>
          ...
          />
          <Axis id="left-axis" position="left" title={"count"} />
        </Chart>
      </div>
    );
  }
