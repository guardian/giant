        ...
        {defaultCollection && (
          <>
            {workspacesMetadata.length > 0 && (
              <EuiFlexGroup>
                <EuiFormControlLayout
                  prepend={
                    <EuiFormLabel htmlFor={"workspace-picker"}>
                      Workspace
                    </EuiFormLabel>
                  }
                >
                  <EuiSelect
                    ...
                    id={"workspace-picker"}
                    options={[{ value: "all", text: "All workspaces" }].concat(
                      _.sortBy(workspacesMetadata, (w) => w.name).map(
                        (w: WorkspaceMetadata) => ({
                          value: w.name,
                          text: w.name,
                        }),
            ...
            <IngestionEvents
              ...
              workspaces={workspacesMetadata.filter(
                (w) =>
                  currentWorkspace === "all" || w.name === currentWorkspace,
              )}
