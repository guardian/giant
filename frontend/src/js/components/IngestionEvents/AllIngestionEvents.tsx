...
function getCollection(collectionId: string, collections: Collection[]) {
  return collections.find(
    (collection: Collection) => collection.uri === collectionId,
  );
}
...
export function AllIngestionEvents({
  ...
  updateCurrentIngestion,
  collections,
  workspacesMetadata,
  currentCollection,
  currentIngestion = "all",
}: {
  ...
  const [toggleIdSelected, setToggleIdSelected] = useState<FilterState>(
    FilterState.All,
  );
  ...
  const collectionOptions: EuiSelectOption[] = _.sortBy(collections, (c) =>
  ...
  useEffect(() => {
    if (currentCollection) {
      ...
      if (
        sc?.ingestions &&
        sc?.ingestions.find((i) => i.display === currentIngestion) === undefined
      ) {
          ...
          {collections.length > 0 && (
            <EuiFlexItem grow={false}>
              <EuiFormControlLayout
                ...
                prepend={
                  <EuiFormLabel htmlFor={"collection-picker"}>
                    Collection
                  </EuiFormLabel>
                }
              >
          ...
          {ingestOptions && (
            <EuiFlexItem grow={false}>
              <EuiFormControlLayout
                className={styles.dropdown}
                prepend={
                  <EuiFormLabel htmlFor={"ingest-picker"}>Ingest</EuiFormLabel>
                }
              >
                <EuiSelect
                  value={currentIngestion}
                  onChange={(e) => updateCurrentIngestion(e.target.value)}
...
function mapDispatchToProps(dispatch: GiantDispatch) {
  return {
    ...
    updateCurrentCollection: bindActionCreators(
      updateCurrentCollection,
      dispatch,
    ),
    updateCurrentIngestion: bindActionCreators(
      updateCurrentIngestion,
      dispatch,
    ),
  };
