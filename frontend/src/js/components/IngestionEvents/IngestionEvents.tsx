...
const blobStatusIcons = {
  ...
  infiniteLoop: (
    <EuiIconTip type="alert" content={"Ingestion failing in infinite loop"} />
  ),
};
...
const getFailedStatuses = (statuses: ExtractorStatus[]) =>
  statuses.filter(
    (status) =>
      status.statusUpdates.find((u) => u.status === "Failure") !== undefined,
  );

const getFailedBlobs = (blobs: BlobStatus[]) => {
  return blobs.filter((wb) => {
    return (
      getFailedStatuses(wb.extractorStatuses).length > 0 || wb.infiniteLoop
    );
  });
};
...
const columns: Array<EuiBasicTableColumn<BlobStatus>> = [
  ...
  },
  {
    ...
    render: (row: BlobStatus) => (
      <>
        {moment
          .duration(moment(row.mostRecentEvent).diff(moment(row.ingestStart)))
          .humanize()}{" "}
          ...
          {statuses.map((status) => {
            const mostRecent =
              status.statusUpdates.length > 0
                ? status.statusUpdates[status.statusUpdates.length - 1]
                : undefined;
            return (
                    ...
                    {mostRecent?.status ? (
                      ...
                      >
                        <EuiBadge color={statusToColor(mostRecent.status)}>
                          {mostRecent.status} (
                          {moment(mostRecent.eventTime).format("HH:mm:ss")})
                        </EuiBadge>
                      </EuiToolTip>
                    ) : (
                      <>No updates</>
    ...
    extractorStatuses: status.extractorStatuses.map((s: any) => ({
      ...
      statusUpdates: _.sortBy(
        s.statusUpdates
          ...
          .filter(
            (u: any) => u.eventTime !== undefined && u.status !== undefined,
          )
          .map((u: any) => ({
            ...u,
...
const renderExpandedRow = (blobStatus: BlobStatus) => {
  ...
  return (
    <EuiText>
      ...
      <p>
        Full file path(s) : {blobStatus.paths.join(", ")}. Ingestion started on{" "}
        {hdate.prettyPrint(blobStatus.ingestStart)}
      </p>
            ...
            {blobStatus.extractorStatuses.map((extractorStatus) => {
              ...
              return (
                <>
                  ...
                  <p>
                    The extractor {extractorStatus.extractorType} has been
                    started {numStarted} times. There have been {numErrors}{" "}
                    errors.
                    <br />
                    {mostRecent ? (
                      <>
                        The most recent status event is '{mostRecent.status}'
                        which happened on{" "}
                        {hdate.prettyPrint(mostRecent.eventTime, {
                          showTime: true,
                        })}
                      </>
                    ) : (
