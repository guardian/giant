...
export class CreateNewUserUnconnected extends React.Component<any, any> {
  static propTypes = {
    config: config,
  ...
  pandaContinue() {
    return this.props.config.userProvider === "panda" && this.state.email;
  }

  canContinue = () => {
    return (
      (this.databaseContinue() || this.pandaContinue()) &&
  ...
  errors = () => {
    ...
    if (this.props.config.userProvider === "database") {
      if (
        this.props.existingUsers.some(
          (existingUser) => existingUser.username === this.state.username,
        )
      ) {
      ...
      if (
        this.props.existingUsers.some(
          (existingUser) => existingUser.username === this.state.email,
        )
      ) {
        errors.push("The user already exists");
        return errors;
    ...
    return (
      <form className="form" onSubmit={this.submit}>
        ...
        {this.renderField(
          ...
          "Username",
          "text",
          this.removeWhitespaceAndUpdateState,
          undefined,
          true,
        )}
        {this.renderField(
