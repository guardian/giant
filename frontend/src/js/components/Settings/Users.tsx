...
import { MdDone as MdDone } from "react-icons/md";
...
class Users extends React.Component<Props, State> {
  ...
  UNSAFE_componentWillReceiveProps = (nextProps: Props) => {
    if (
      this.state.creatingUserModal &&
      this.props.creatingUser &&
      !nextProps.creatingUser &&
      nextProps.createUserErrors.length === 0
    ) {
  ...
  confirmDeleteModal = () => {
    return (
      <Modal
        isOpen={this.state.deletingUsername !== undefined}
        dismiss={this.onDismissDeleteModal}
      >
        <div>
          <h2 className="modal__title">
            Really delete user {this.state.deletingUsername}?
          </h2>
          <button
            className="btn"
            onClick={() => this.deleteUser(this.state.deletingUsername)}
          >
            OK
          </button>
          <button className="btn" onClick={this.onDismissDeleteModal}>
            Cancel
          </button>
        </div>
      </Modal>
        ...
        <div>
          <div className="btn-group btn-group--left btn-group--bottom-padding">
            <button
              className="btn"
              name="creatingUser"
              onClick={() => this.setState({ creatingUserModal: true })}
            >
              Create New User
            </button>
          ...
          <Modal
            isOpen={this.isCreatingUser()}
            dismiss={() => this.setState({ creatingUserModal: false })}
          >
            <CreateNewUser />
          </Modal>
        </div>
  ...
  savePermissions(username: string, canManageUsers: boolean) {
    this.props.setUserPermissions(
      username,
      canManageUsers ? ["CanPerformAdminOperations"] : [],
    );
  }
  ...
  renderUser = ({
    ...
    const canWeManageUsers = this.props.myPermissions.includes(
      "CanPerformAdminOperations",
    );
    const canTheyManageUsers = granted.includes("CanPerformAdminOperations");
    const canWeManageThisUser =
      !this.myUser(username) &&
      this.props.myPermissions.includes("CanPerformAdminOperations");

    return (
      <tr
        className={`data-table__row ${canWeManageThisUser ? "data-table__row--hover" : ""}`}
        key={username}
      >
        <td className="data-table__item data-table__item--value">{username}</td>
        ...
        ) : null}
        {
          <td className="data-table__item data-table__item--value">
            <Checkbox
              disabled={!canWeManageThisUser}
              selected={canTheyManageUsers}
              onClick={() =>
                this.savePermissions(username, !canTheyManageUsers)
              }
            >
              Admin
            </Checkbox>
        ...
        <table className="data-table">
          ...
          <tbody>
            {this.props.users.length ? (
              _sortBy(this.props.users, ({ username }) =>
                username.toLowerCase(),
              ).map(this.renderUser)
            ) : (
              <tr className="data-table__row">
                <td className="data-table__item" colSpan={3}>
                  No results found
                </td>
              </tr>
            )}
