...
class ExtractionFailuresComponent extends React.Component<Props, State> {
  ...
  renderRow = (failure: ExtractionFailureSummary) => {
    ...
    return (
      <tr
        ...
        key={`${failure.extractorName}-${failure.stackTrace}`}
      >
        ...
        <td className="data-table__item data-table__item--value">
          ...
          {expanded ? (
            <ResourcesForExtractionFailureComponent summary={failure} />
          ) : (
            false
          )}
        </td>
        <td className="data-table__item data-table__item--value">
          <button
            className="btn"
            onClick={() => this.toggleExpanded(key, expanded)}
          >
  ...
  renderNoResults = () => {
    return (
      <tr className="data-table__row">
        <td className="data-table__item" colSpan={4}>
          No results found
        </td>
      </tr>
    );
  ...
  render() {
    if (
      this.props.extractionFailures &&
      this.props.extractionFailures.results
    ) {
      return (
          ...
          <table className="data-table" style={{ tableLayout: "fixed" }}>
            ...
            <tbody>
              {this.props.extractionFailures.results.length
                ? this.props.extractionFailures.results.map((failure) =>
                    this.renderRow(failure),
                  )
