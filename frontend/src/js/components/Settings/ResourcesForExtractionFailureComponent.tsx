...
export function ResourcesForExtractionFailureComponent({
  summary,
}: {
  summary: ExtractionFailureSummary;
}) {
  const [resources, setResources] = useState<
    ResourcesForExtractionFailure | undefined
  >(undefined);
  const [page, setPage] = useState(1);

  useEffect(() => {
    fetchResourcesForExtractionFailure(
      summary.extractorName,
      summary.stackTrace,
      page,
    ).then((resources) => {
    ...
    <table className="data-table" style={{ tableLayout: "fixed" }}>
      <tbody>
        {resources.results.map(({ uri, parents }) => {
          const displayUri =
            parents && parents.length > 0 ? parents[0].uri : uri;

          return (
            <tr key={uri} className="data-table__row">
              <td className="data-table__item data-table__item--value">
                <SearchLink to={`/viewer/${uri}`}>{displayUri}</SearchLink>
              </td>
            </tr>
          );
        })}
