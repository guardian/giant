...
function transformQuery(q: any[]) {
  return q.map((fragment: any) => {
    if (_isObject(fragment)) {
      const frag = fragment as any;
      if (frag.t === "date") {
        const parsed = parseDate(frag.v, "from_start");
        if (parsed) {
          return Object.assign({}, frag, { v: parsed.toString() });
        } else {
          // If the date chip is invalid just ignore it
          return "";
        }
      } else if (frag.t === "date_ex") {
        const parsed = parseDate(frag.v, "from_end");
        if (parsed) {
          return Object.assign({}, frag, { v: parsed.toString() });
        } else {
          // If the date chip is invalid just ignore it
...
export function performSearch(searchQuery: any) {
  ...
  return authFetch(`/api/search?${objectToParamString(queryObject)}`).then(
    (res) => res.json(),
  );
}
