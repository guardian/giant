...
export function authPreviewLink(uri: string, filename?: string) {
  if (filename) {
...
export function getPreviewType(uri: string) {
  return authFetch(`/api/preview/download/${uri}`, { method: "HEAD" }).then(
    (r) => {
      if (r.status === 200) {
...
export function getPreviewBlob(uri: string) {
...
export function fetchPreviewLink(uri) {
  return authFetch(`/api/preview/generate/${uri}`, { method: "POST" }).then(
    () => {
      return authFetch(authPreviewLink(uri), {
