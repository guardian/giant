import { ColumnsConfig, Tree, TreeEntry, TreeLeaf, TreeNode } from '../types/Tree';
...
const columnsConfig: ColumnsConfig<null> = {
    ...
    columns: [{
        ...
        sort: (a: TreeEntry<null>, b: TreeEntry<null>) => a.name.localeCompare(b.name)
    } as const]
};
...
test('treeToOrderedEntries', () => {
    ...
    expect(
        treeToOrderedEntries(tree, columnsConfig, [])
    ).toStrictEqual([a, d]);

    expect(
        treeToOrderedEntries(tree, columnsConfig, [a])
    ).toStrictEqual([a, b, c, d]);

    expect(
        treeToOrderedEntries(tree, columnsConfig, [a, e])
    ).toStrictEqual([a, b, c, d]);

    expect(
        treeToOrderedEntries(tree, columnsConfig, [a, d])
    ).toStrictEqual([a, b, c, d, e]);

    expect(
        treeToOrderedEntries(tree, columnsConfig, [a, d, e])
    ...
    expect(
        treeToOrderedEntries(tree.map(obj => cloneDeep(obj)), columnsConfig, [a, d, e])
    ).toStrictEqual([a, b, c, d, e, f]);
...
test('getShiftClickSelectedEntries', () => {
    expect(getShiftClickSelectedEntries(
        [a, b, c, d, e, f],
        b,
        d
    )).toStrictEqual([b, c, d]);
...
describe('newSelectionFromShiftClick', () => {
    test('re-defining a shift-click selection', () => {
        expect(newSelectionFromShiftClick(
            [a,b,c],
            [d,e],
            []
        )).toStrictEqual([d,e]);
