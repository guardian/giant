...
export const MAX_NUMBER_OF_CHILDREN = 5000;

export function hasSingleBlobChild(
  resource: BasicResource,
): resource is BasicResourceWithSingleBlobChild {
  return !!(
    resource.type === "file" &&
    resource.children &&
...
export function canLinkTo(resource: BasicResource): boolean {
  return (
    resource.type === "email" ||
    (resource.type === "file" && !resource.isExpandable)
  );
}
...
export function definitelyNotAUnifiedViewer(
  resource: Resource | null,
): { view: string; highlightableText: HighlightableText } | undefined {
  if (!resource) {
    return undefined;
  }
  const ocrWithHighlights =
    resource.ocr &&
    Object.entries(resource.ocr).find(
...
export function getCurrentHighlight(
  ...
  if (toDisplay) {
    return highlights[`${resource.uri}-${query}`]?.[toDisplay.view]
      .currentHighlight;
  }
  return undefined;
}
