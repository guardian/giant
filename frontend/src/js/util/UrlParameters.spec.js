import {objectToParamString, paramStringToObject, isValidValue} from './UrlParameters';
...
test('objects are converted correctly', () => {
    ...
    expect(objectToParamString(obj3)).toBe('testValue=test&testArray[]=test1&testArray[]=test2');
    expect(objectToParamString(obj4)).toBe('testValue=test');
    expect(objectToParamString(obj5)).toBe('');
    expect(objectToParamString(obj6)).toBe('testValue=%F0%9F%91%BB%F0%9F%90%B8&testArray[]=%E6%B5%8B%E8%AF%95&testArray[]=%E0%B8%97%E0%B8%94%E0%B8%AA%E0%B8%AD%E0%B8%9A');
    expect(objectToParamString(obj7)).toBe('tes%26t=te%3F%3F%3Ft&testArray[]=%26&testArray[]=%26%20%26');
    expect(objectToParamString(obj8)).toBe('testValue=test&testObject.testObjectValue[]=test1&testObject.testObjectValue[]=test2&testObject.testObjectValue2=a%20string');
...
test('Param string correctly becomes an object', () => {
    ...
    const testString4 = '?filters.ingestion[]=Test%20Ingest&filters.ingestion[]=Test%20Ingest2';
