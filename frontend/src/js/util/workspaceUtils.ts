...
export const findPath = (
    ...
    if (isTreeNode(currentNode)) {
        for (let i = 0; i < currentNode.children.length; i++) {
            const v = findPath(targetId, R.append(currentNode, currentParents), currentNode.children[i]);
            if (v !== null) {
                return v;
            }
        }
...
export const displayRelativePath = (root: TreeNode<WorkspaceEntry>, nodeId: string): string => {
...
export function workspaceHasProcessingFiles(workspace: Workspace): boolean {
    function entriesHaveProcessingFiles(entries: TreeEntry<WorkspaceEntry>[]): boolean {
        for (const entry of entries) {
            ...
            } else if (isTreeLeaf(entry) && isWorkspaceLeaf(entry.data) && entry.data.processingStage.type === 'processing') {
                return true;
            }
        }
        return false;
...
export function processingStageToString(processingStage: ProcessingStage): string {
    switch(processingStage.type) {
        case 'processing': {
            const description = processingStage.tasksRemaining === 1
                ? '1 task remaining'
