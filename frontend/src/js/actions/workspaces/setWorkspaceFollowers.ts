import { updateWorkspaceFollowers } from '../../services/WorkspaceApi';
...
export function setWorkspaceFollowers(
    id: string,
    followers: string[]
): ThunkAction<void, GiantState, null, WorkspacesAction | AppAction> {
    return dispatch => {
        return updateWorkspaceFollowers(id, followers)
            .then(res => {
                dispatch(getWorkspacesMetadata());
                // We need to fire off this second call because followers metadata as
