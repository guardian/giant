...
function GiantEuiSettings({ preferences, updatePreference }: { preferences: any, updatePreference: (id: string, value: boolean) => void }) {
    const options = features.map(({ name, description}) => {
        return { id: name, label: description };
    });

    const selected = features.reduce((acc, { name }) => {
