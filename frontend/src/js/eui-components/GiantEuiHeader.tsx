...
import { EuiAvatar, EuiFieldSearch, EuiHeader, EuiHeaderLogo, EuiHeaderSectionItemButton, EuiIcon } from '@elastic/eui';
...
function GiantEuiHeader({
    ...
    const search = (
        <React.Fragment>
            <EuiFieldSearch
                style={{width: '250px'}}
                placeholder={currentWorkspace ? `Search in workspace ${currentWorkspace.name}` : 'Search anywhere'}
                aria-label={currentWorkspace ? `Search in workspace ${currentWorkspace.name}` : 'Search anywhere'}
                value={searchTerm}
                ...
                onSearch={ (searchTerm: string) => {
                    ...
                    if (workspaceId) {
                        searchQuery = { ...searchQuery, ...{ filters:  { workspace: [workspaceId] } } };
                    }
    ...
    const sections = [
        {
            items: [<GiantEuiLeftHandNav workspacesMetadata={workspacesMetadata} />, logo],
