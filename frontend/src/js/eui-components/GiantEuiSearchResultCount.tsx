...
function GiantEuiSearchResultCount({resource, currentQuery, highlights}: Props) {
    let currentHighlight, totalHighlights;
    if (highlights && currentQuery && resource) {
        currentHighlight = getCurrentHighlight(highlights, resource, currentQuery.q);
        totalHighlights = getTotalHighlights(resource);
    }

    if (totalHighlights && totalHighlights > 0 && currentHighlight !== undefined) {
        return <span>{currentHighlight + 1} of {totalHighlights}</span>;
    }
