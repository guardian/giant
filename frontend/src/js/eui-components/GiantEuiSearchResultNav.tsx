...
function scrollToHighlight(highlightNumber: number) {
  ...
  if (highlights[highlightNumber]) {
    ...
    console.error(
      `Could not find element number ${highlightNumber} in highlights of length ${highlights.length}`,
    );
  }
}

function GiantEuiSearchResultNav({
  resource,
  currentQuery,
  setCurrentHighlight,
  highlights,
  ...
  useEffect(() => {
    if (resource && currentQuery && toDisplay) {
      const currentHighlight =
        getCurrentHighlight(highlights, resource, currentQuery.q) ?? 0;
  ...
  function nextHighlight() {
    ...
    const previousHighlight: number =
      getCurrentHighlight(highlights, resource, currentQuery.q) ?? -1;
    const newHighlight: number =
      (previousHighlight + 1) % (getTotalHighlights(resource) ?? 0);
