...
import com.gu.pandomainauth.service.CookieUtils.CookieIntegrityFailure.MalformedCookieText
import com.gu.pandomainauth.service.{CookieUtils, KeyPair}
import com.gu.pandomainauth.service.CryptoConf.SettingsReader.{privateKeyFor, publicKeyFor}
import com.gu.pandomainauth.service.CryptoConf.{SigningAndVerification, Verification}
...
class PanDomainUserProviderTest extends AnyFreeSpec with Matchers with AttemptValues with Results  {
  ...
  "PanDomainUserProviderTest" - {
    ...
    "clientConfig is correctly populated" in {
      ...
      val provider = new PanDomainUserProvider(config, verificationProvider, TestUserManagement(Nil), metricsService)
    ...
    "authenticate" - {
      ...
      "fails when cookie is garbage" in {
        ...
        result.failureValue shouldBe PanDomainCookieInvalid(s"Pan domain cookie invalid: $MalformedCookieText", reportAsFailure = true)
      }
      ...
      "succeeds when cookie is valid" in {
        ...
        val config = PandaAuthConfig("bob", "bob.key", "bobCookie", require2FA = true, "https://login.bob.example/login", AwsConnection("eu-west-1", None))
        val provider = new PanDomainUserProvider(config, verificationProvider, TestUserManagement(List(bob)), metricsService)
        val result = provider.authenticate(
          FakeRequest("GET", "/random").withCookies(Cookie("bobCookie", privateCookie)),
          now
        )
      ...
      "registers a user on first access" in {
        ...
        val provider = new PanDomainUserProvider(config, verificationProvider, users, metricsService)
