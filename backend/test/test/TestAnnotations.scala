...
class TestAnnotations(usersToWorkspaces: Map[String, List[String]] = Map.empty) extends Annotations {
  ...
  override def getWorkspaceChildren(currentUser: String, workspaceId: String, maybeParentId: Option[String]): Attempt[TreeEntry[WorkspaceEntry]] = Attempt.Left(UnsupportedOperationFailure(""))
  override def getWorkspaceAncestors(currentUser: String, workspaceId: String, nodeId: String): Attempt[List[String]] = Attempt.Left(UnsupportedOperationFailure(""))
