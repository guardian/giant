...
/*
  ...
  */
    ...
    def process(flag: OcrMyPdfFlag, previousExitCode: Option[Int] = None, overrideFile: Option[Path] = None): Int = {
      val exitCode = ocrWithOcrMyPdf(flag, overrideFile)
      exitCode match {
        case 2 if !previousExitCode.contains(2) =>
          ...
          process(SkipText, Some(exitCode), overrideFile)
        case 8 if !previousExitCode.contains(8) =>
          ...
          if (qpdfResult) {
            process(RedoOcr, Some(exitCode), Some(decryptTempFile))
          } else {
            exitCode
          }
        case _ => exitCode
    ...
    val exitCode = process(initialFlag)
