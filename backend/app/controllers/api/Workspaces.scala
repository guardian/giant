...
case class AddRemoteUrlData(url: String, title: String, parentFolderId: String, collection: String, ingestion: String)
...
class Workspaces(override val controllerComponents: AuthControllerComponents, annotation: Annotations, index: Index, manifest: Manifest,
  ...
  def addRemoteUrlToWorkspace(workspaceId: String) = ApiAction.attempt(parse.json) { req =>
    for {
