...
class AppComponents(context: Context, config: Config)
  ...
  val router = try {
    ...
    val collectionsController = new Collections(authControllerComponents, manifest, users, esResources, config.s3, esEvents, esPages, ingestionServices, annotations, postgresClient)
