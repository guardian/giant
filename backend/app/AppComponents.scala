...
class AppComponents(context: Context, config: Config)
  ...
  val router = try {
    ...
    val workspacesController = new Workspaces(authControllerComponents, annotations, esResources, manifest, users, blobStorage, previewStorage, postgresClient, remoteIngestStore, ingestStorage, config.mediaDownload, sqsClient)
