...
trait PostgresClient {
  ...
  def getRemoteIngestJob(id: String): Either[GiantFailure, Option[RemoteIngest]]
}
...
class PostgresClientDoNothing extends PostgresClient {
  ...
  def getRemoteIngestJob(id: String): Either[GiantFailure, Option[RemoteIngest]] = Right(None)
...
class PostgresClientImpl(postgresConfig: PostgresConfig) extends PostgresClient with Logging {
  ...
  def getRemoteIngestJob(id: String): Either[GiantFailure, Option[RemoteIngest]] = {
    Try {
      ...
      case Success(remoteIngest) => Right(remoteIngest)
      case Failure(exception) =>
