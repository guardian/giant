        ...
        (for {
          ...
          _ <- remoteIngestStore.updateRemoteIngestJobStatus(remoteIngestOutput.id, remoteIngestOutput.taskId, RemoteIngestStatus.Completed)
            ...
            failureDetail => {
              ...
              logger.error(s"Failed to ingest remote file for job with id ${maybeParsedJob.map(_.id).getOrElse(s"unknown (but had sqs id ${sqsMessage.getMessageId}")}", failureDetail.toThrowable)
